...
def heatingSetpointUp() {
    ...
    if (newHeatingSetpoint) {
      ...
      log.info "Setpoint": newHeatingSetpoint
...
def setTemperature(value) {
    ...
    log.debug "Setpoint": device.latestValue("heatingSetpoint")

    def sp = device.latestValue("heatingSetpoint")
    int diff = value - sp
    def cool
    if (diff < 0) cool = "$diff"
    if (diff == 0) cool = '0'
    if (diff > 0) cool = "+$diff"
    sendEvent(name: 'coolingSetpoint', value: cool)
    sendEvent(name: 'heatingSetpoint', value: device.latestValue("heatingSetpoint"))
...
def setHeatingSetpoint(value) {
	log.info "Setpoint received": value
